...
def test_write_note_creates_file(inbox):
    ...
    assert path.name == "260715-노트-테스트_노트.md"
...
def test_write_note_creates_missing_inbox(tmp_path):
    ...
    path = write_note(inbox_dir=target_dir, body="x", title="t", when=_dt(), category="노트")
    assert path.exists()
    assert path.parent == target_dir


def test_write_note_no_tmp_litter(inbox):
    write_note(inbox_dir=inbox, body="x", title="t", when=_dt(), category="노트")
    assert not list(inbox.glob("*.tmp"))


def test_write_note_collision(inbox):
    p1 = write_note(inbox_dir=inbox, body="a", title="같은 제목", when=_dt(), category="노트")
    p2 = write_note(inbox_dir=inbox, body="b", title="같은 제목", when=_dt(), category="노트")
    assert p1 != p2
